...
-- Fonte unica eventi: Vercel API server -> Supabase service_role -> public.codm_events.
-- La PWA/browser NON devono più passare clan_id, NON devono usare localStorage eventi, NON devono usare fallback RLS per eventi.
...
-- Reset metadata legacy PWA. Gli eventi reali restano, ma non esiste più stato locale/pending.
...
create index if not exists idx_codm_events_created_at on public.codm_events(created_at desc);
...
-- RLS rimane come sicurezza secondaria, ma V8.0 eventi usa service_role lato server.
...
-- Controllo manuale consigliato dopo deploy:
-- select id, clan_id, title, starts_at, event_plan->>'teamBName' as avversario, local_id, sync_status from public.codm_events order by created_at desc;

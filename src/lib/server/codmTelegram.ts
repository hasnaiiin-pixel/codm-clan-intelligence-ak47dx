...
function eventDate(value: unknown) {
  ...
  return escapeHtml(date.toLocaleString('it-IT', { dateStyle: 'full', timeStyle: 'short', timeZone: 'Europe/Rome' }));
}
...
function eventTime(value: unknown) {
  ...
  if (!Number.isNaN(date.getTime())) return escapeHtml(date.toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit', timeZone: 'Europe/Rome' }));
  return escapeHtml(text);
}

function planOf(event: TelegramEvent): MatchPlan {
  return event?.event_plan && typeof event.event_plan === 'object' ? event.event_plan : {};
}
...
function teamBName(event: TelegramEvent) {
  ...
  return String(plan.teamBName || plan.opponentName || 'Clan avversario').trim() || 'Clan avversario';
}
...
function renderRound(round: MatchRound, index: number, plan: MatchPlan) {
  ...
  const location = round.location || plan.roomNumber || plan.lobbyLink || 'Private lobby / da confermare';
  ...
  const score = round.ourScore || round.opponentScore ? `${round.ourScore || '-'} - ${round.opponentScore || '-'}` : '';
  if (score || round.result || round.mvp) {
    lines.push('', `🏆 <b>Risultato:</b> ${compact(round.result || score, 'Da giocare')}`);
...
function fallbackConvocations(event: TelegramEvent) {
  const starters = (event.convocations || []).filter((p) => String(p.role || '').toLowerCase().includes('titol'));
  const reserves = (event.convocations || []).filter((p) => String(p.role || '').toLowerCase().includes('ris'));
  if (!starters.length && !reserves.length && event.convocations_text) {
    return `👥 <b>Convocati:</b>\n${escapeHtml(event.convocations_text)}`;
  }
  const starterText = starters.length ? starters.map((p, i) => `${i + 1}. ${escapeHtml(p.nickname || '-')}`).join('\n') : 'Da confermare';
  const reserveText = reserves.length ? reserves.map((p, i) => `${i + 1}. ${escapeHtml(p.nickname || '-')}`).join('\n') : 'Da confermare';
  return `👥 <b>Titolari:</b>\n${starterText}\n\n🔁 <b>Riserve:</b>\n${reserveText}`;
}

function eventNotes(event: TelegramEvent) {
  const notes = String(event.event_notes || event.description || '').replace(/AK_EVENT_PLAN_V[\d_]+::[\s\S]*$/g, '').trim();
  return notes;
}
...
export function renderProfessionalEventTelegram(
  ...
  const plan = planOf(event);
  const rounds = Array.isArray(plan.rounds) && plan.rounds.length ? plan.rounds : [];
  ...
  if (plan.lobbyTime) lines.push(`🎧 <b>Apertura lobby:</b> ${escapeHtml(plan.lobbyTime)}`);
  if (plan.roomNumber) lines.push(`🔢 <b>Room ID:</b> ${escapeHtml(plan.roomNumber)}`);

  if (rounds.length) {
    rounds.forEach((round, index) => {
      lines.push('', '━━━━━━━━━━━━━━━━━━━━', '', renderRound(round, index, plan));
    });
  } else {
...
export async function sendTelegramReminder(event: TelegramEvent, minutes: number): Promise<TelegramResult> {
  if (event?.telegram_enabled === false) return { ok: true, skipped: true };
  const mode = minutes === 0 ? 'started' : 'reminder';
  return sendTelegramHtml(renderProfessionalEventTelegram(event, mode, { minutes }));
}

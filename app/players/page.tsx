...
export default function PlayersPage() {
  ...
  async function load() {
    ...
    const { data: snapData } = await supabase.from('player_snapshots').select('*').order('imported_at', { ascending: false });
    const { data: statData } = await supabase.from('match_player_stats').select('*, matches(mode,result,map_name,match_date)').order('created_at', { ascending: false });
    ...
    setClanEdits(Object.fromEntries(loadedPlayers.map((p) => [p.id, p.clan_name || ''])));
  }
  ...
  async function addPlayer() {
    ...
    const { error } = await supabase.from('players').insert({
      ...
      notes: uid ? null : 'Player creato manualmente o da statistiche: profilo CODM non ancora collegato.'
    });
    if (error) return setMessage(error.message);
    setNickname(''); setUid(''); setRankMp(''); setRankBr('');
    setMessage('Player aggiunto. Le statistiche future saranno associate a questo nome/clan.');
    load();
  }

  async function updatePlayerClan(player: Player) {
    const nextClan = (clanEdits[player.id] || '').trim() || null;
    const { error } = await supabase.from('players').update({ clan_name: nextClan }).eq('id', player.id);
    if (error) return setMessage(`Errore aggiornamento clan ${player.nickname}: ${error.message}`);
    setMessage(`Clan aggiornato per ${player.nickname}: ${nextClan || 'Senza clan'}.`);
    load();
  }
  ...
  const playerCards = useMemo<PlayerCard[]>(() => {
    ...
    return players
      ...
      .filter((p) => !q || `${p.nickname} ${p.clan_name || ''} ${p.uid_codm || ''}`.toLowerCase().includes(q))
      .map((p) => {
        const pStats = stats.filter((s) => s.player_id === p.id);
        const pSnapshots = snapshots.filter((s) => s.player_id === p.id);
        const mpSnapshot = pSnapshots.find((s) => s.source_type === 'multiplayer');
        const brSnapshot = pSnapshots.find((s) => s.source_type === 'battle_royale');
        ...
        const rankValues = pStats.map((s) => s.rank_position || 0).filter((x) => x > 0);
        const rankAvg = rankValues.length ? rankValues.reduce((a, b) => a + b, 0) / rankValues.length : 0;
        return {
          player: p,
      ...
      .sort((a, b) => b.kills - a.kills || a.player.nickname.localeCompare(b.player.nickname));
  }, [players, stats, snapshots, filterClan, search]);

  const clanSummary = useMemo(() => {
    const grouped = new Map<string, { clan: string; players: number; kills: number; deaths: number; assists: number; matches: number; gold: number; silver: number; bronze: number; rankSum: number; rankCount: number }>();
    for (const p of players) {
      const clan = safeClan(p);
      const item = grouped.get(clan) || { clan, players: 0, kills: 0, deaths: 0, assists: 0, matches: 0, gold: 0, silver: 0, bronze: 0, rankSum: 0, rankCount: 0 };
      item.players += 1;
      const pStats = stats.filter((s) => s.player_id === p.id);
    ...
    return Array.from(grouped.values()).map((c) => ({ ...c, kd: kdRatio(c.kills, c.deaths), avgRank: c.rankCount ? (c.rankSum / c.rankCount).toFixed(1) : '-' })).sort((a, b) => b.kills - a.kills);
  }, [players, stats]);
    ...
    <main className="container wide">
      <section className="grid grid-2">
        <div className="card gaming-panel">
          ...
          <p className="muted">Qui vedi player registrati e manuali. Puoi assegnare o correggere il clan, mantenendo le statistiche importate. In futuro un player manuale può essere collegato a un profilo completo.</p>
          <div className="form">
            ...
            <button className="btn" onClick={addPlayer}>➕ Aggiungi player</button>
        ...
        <div className="card">
          ...
          <div className="grid grid-2">
            <div className="field"><label>Filtro clan</label><select className="select" value={filterClan} onChange={(e) => setFilterClan(e.target.value)}>{clanOptions.map((c) => <option key={c} value={c}>{c === 'ALL' ? 'Tutti i clan' : c}</option>)}</select></div>
          ...
          <table className="table compact player-stats-table">
            ...
            <tbody>{playerCards.map((card) => (
              <tr key={card.player.id}>
                ...
                <td><span className={card.player.uid_codm ? 'badge ok' : 'badge warn'}>{profileStatus(card.player)}</span></td>
                <td>{card.matchCount}</td>
                <td>{card.wins}/{Math.max(card.matchCount - card.wins, 0)}</td>
                <td>{card.winRate}%</td>
                <td>{card.kills}</td><td>{card.deaths}</td><td>{card.assists}</td><td>{card.kd}</td>
                <td><span className="rank-medal medal-gold">🥇 {card.gold}</span></td>
                ...
                <td>{card.avgRank}</td>
                <td><div className="inline-edit"><input className="input clan-edit" value={clanEdits[card.player.id] ?? ''} onChange={(e) => setClanEdits((current) => ({ ...current, [card.player.id]: e.target.value }))} placeholder="AK / AP" /><button className="btn small secondary" onClick={() => updatePlayerClan(card.player)}>Salva</button></div></td>
              </tr>

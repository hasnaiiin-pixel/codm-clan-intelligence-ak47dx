...
const modeOptions: ModeOption[] = [
  ...
  { value: 'KILL_CONFIRMED', label: '🏷️ Kill Confirmed', icon: '🏷️', help: 'Dog tags' },
  { value: 'BR', label: '🪂 Battle Royale', icon: '🪂', help: 'Battle Royale' },
  { value: 'SCRIM', label: '🎮 Scrim libero', icon: '🎮', help: 'Modalità scrim' },
];

const codmMaps = [
  'Standoff', 'Raid', 'Firing Range', 'Summit', 'Slums', 'Hacienda', 'Takeoff', 'Meltdown', 'Crash', 'Crossfire', 'Nuketown', 'Nuketown Russia', 'Hijacked', 'Shoot House', 'Shipment', 'Rust', 'Terminal', 'Highrise', 'Hackney Yard', 'Tunisia', 'Coastal', 'Express', 'Dome', 'Vacant', 'Scrapyard', 'Monastery', 'Khandor Hideout', 'Suldal Harbor', 'Arsenal', 'Diesel', 'Sawmill', 'Apocalypse', 'Miami Strike', 'Satellite', 'Favela', 'Yemen', 'Kurohana Metropolis', 'Collateral Strike', 'Armada Strike', 'Seaside', 'Icebreaker', 'Frequency', 'Contraband', 'Arklov Peak', 'Oasis', 'Pine', 'Cage', 'Reclaim', 'Aniyah Incursion', 'Alcatraz', 'Isolated', 'Blackout'
];

const scoreTypeOptions: ScoreTypeOption[] = [
  { value: 'Punteggio round', label: '🏁 Punteggio round', target: '6 round', help: 'CED / Control / modalità a round' },
  { value: 'Punteggio', label: '📊 Punteggio', target: '250 punti', help: 'Postazione / Dominio / punti obiettivo' },
  { value: 'Kill', label: '💀 Kill', target: '50 kill', help: 'TDM / Death match' },
  { value: 'Best of 3', label: '🥇 Best of 3', target: '2 mappe vinte', help: 'Serie BO3' },
  { value: 'Best of 5', label: '🏆 Best of 5', target: '3 mappe vinte', help: 'Serie BO5' },
  { value: 'Battle Royale', label: '🪂 BR piazzamento', target: 'Top / kill', help: 'BR con piazzamento e kill' },
];
...
function readPlan(event: CodmEvent): MatchPlan {
  ...
  if (marker) {
    ...
    try { const parsed = JSON.parse(note.slice(idx + marker.length)); return { ...emptyPlan('AK47DX'), ...parsed, rounds: (parsed.rounds || [emptyRound(1)]).map(sanitizeRound) }; } catch {}
  }
  return emptyPlan('AK47DX');
}
function stripOldPlan(notes: string) { let output = notes || ''; for (const marker of [PLAN_MARKER, ...OLD_PLAN_MARKERS]) { const idx = output.indexOf(marker); if (idx >= 0) output = output.slice(0, idx).trim(); } return output.trim(); }
function planNote(plan: MatchPlan, notes: string) { return `${stripOldPlan(notes)}\n\n${PLAN_MARKER}${JSON.stringify(plan)}`.trim(); }
function listFromText(value: string) { return String(value || '').split(/[\n,;]+/).map((item) => item.trim()).filter(Boolean); }
function textFromList(values: string[]) { return Array.from(new Set(values.map((item) => item.trim()).filter(Boolean))).join(', '); }
...
export default function EventsPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        setTitle(draft.title || 'Scrim AK47DX vs Clan avversario'); setDescription(draft.description || ''); setLocation(draft.location || 'CODM room'); setEventType(draft.eventType || 'scrim');
        setStartsAt(draft.startsAt || toLocalInputValue(new Date(Date.now() + 60 * 60 * 1000))); setEndsAt(draft.endsAt || toLocalInputValue(new Date(Date.now() + 3 * 60 * 60 * 1000)));
        setTelegramEnabled(draft.telegramEnabled ?? true); setReminderMinutes(draft.reminderMinutes || '120,60,30,10'); setTelegramTemplate(draft.telegramTemplate || DEFAULT_TELEGRAM_TEMPLATE); setEventNotes(draft.eventNotes || '');
        setSelectedPlayers(Array.isArray(draft.selectedPlayers) ? draft.selectedPlayers : []); setReservePlayers(Array.isArray(draft.reservePlayers) ? draft.reservePlayers : []);
        if (draft.plan) setPlan(normalizePlan({ ...emptyPlan(auth.clanName || 'AK47DX'), ...draft.plan }));
        setEditingEventId(draft.editingEventId || null);
        setMessage('Bozza evento ripristinata: non si perde quando cambi tab Chrome o apri cartelle sul PC.');
      }
    } catch {}
    setDraftReady(true);
  }, []);
  ...
  async function loadEvents() {
    setEventsLoading(true); setMessage((m) => m && m.includes('Bozza') ? m : '');
    ...
    const rows = (data || []) as CodmEvent[]; setEvents(rows);
    const ids = rows.map((e) => e.id).filter(Boolean);
    if (ids.length) { const { data: epRows } = await supabase.from('codm_event_players').select('event_id,player_id,nickname,status').in('event_id', ids); setEventPlayers((epRows || []) as EventPlayerRow[]); } else setEventPlayers([]);
    setEventsLoading(false);
  }

  function parseReminderMinutes() { const values = reminderMinutes.split(',').map((x) => Number(x.trim())).filter((n) => Number.isFinite(n) && n > 0 && n <= 10080); return Array.from(new Set(values.length ? values : [120, 10])).sort((a, b) => b - a); }
  ...
  function loadEventIntoEditor(event: CodmEvent, duplicate = false) { const eventPlan = readPlan(event); setEditingEventId(duplicate ? null : event.id); setTitle(duplicate ? `Copia - ${event.title}` : event.title); setDescription(stripOldPlan(event.description || '')); setLocation(event.location || 'CODM room'); setEventType(event.event_type || 'scrim'); setStartsAt(toLocalInputValue(new Date(event.starts_at))); setEndsAt(event.ends_at ? toLocalInputValue(new Date(event.ends_at)) : ''); setTelegramEnabled(event.telegram_enabled ?? true); setReminderMinutes((event.reminder_minutes || [120,60,30,10]).join(',')); setTelegramTemplate(event.telegram_message_template || DEFAULT_TELEGRAM_TEMPLATE); setEventNotes(stripOldPlan(event.event_notes || '')); setPlan(normalizePlan(eventPlan)); const starters = eventPlayers.filter((r) => r.event_id === event.id && r.status === 'titolare').map((r) => r.player_id).filter(Boolean) as string[]; const reserves = eventPlayers.filter((r) => r.event_id === event.id && r.status === 'riserva').map((r) => r.player_id).filter(Boolean) as string[]; setSelectedPlayers(starters); setReservePlayers(reserves); setMessage(duplicate ? 'Evento duplicato nell’editor: modifica data/orari e salva come nuovo evento.' : 'Evento aperto in modifica dall’elenco/calendario.'); setTimeout(() => document.querySelector('.event-create-v64')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50); }
  ...
  async function createEvent() {
    ...
    const startIso = new Date(startsAt).toISOString(); const endIso = endsAt ? new Date(endsAt).toISOString() : null;
    const convocati = players.filter((p) => selectedPlayers.includes(p.id)); const reserves = players.filter((p) => reservePlayers.includes(p.id));
    const effectivePlan = normalizePlan(plan);
    const matchDetailsText = buildMatchDetails(effectivePlan).replace(/<[^>]*>/g, '');
    const convocationsText = [convocati.length ? `Titolari evento:\n${convocati.map((p) => `• ${p.nickname}`).join('\n')}` : '', reserves.length ? `Riserve evento:\n${reserves.map((p) => `• ${p.nickname}`).join('\n')}` : '', matchDetailsText ? `Dettaglio partite:\n${matchDetailsText}` : ''].filter(Boolean).join('\n\n');
    ...
    const basePayload: Record<string, any> = { clan_id: auth.clanId, title, description: fullDescription || null, location: location || null, event_type: eventType, starts_at: startIso, ends_at: endIso, telegram_enabled: telegramEnabled, reminder_minutes: parseReminderMinutes(), telegram_message_template: telegramTemplate || DEFAULT_TELEGRAM_TEMPLATE, event_notes: planNote(effectivePlan, eventNotes), google_calendar_url: googleUrl, convocations: convocati.map((p) => ({ id: p.id, nickname: p.nickname, role: 'titolare' })).concat(reserves.map((p) => ({ id: p.id, nickname: p.nickname, role: 'riserva' }))), convocations_text: convocationsText || null };
    const payloadWithPlan = { ...basePayload, event_plan: effectivePlan };
    let eventId = editingEventId;
    let error: any = null;
    if (editingEventId) {
      const update = await supabase.from('codm_events').update(payloadWithPlan).eq('id', editingEventId).select('id').single();
      error = update.error; if (error && /event_plan|column/i.test(error.message)) { const retry = await supabase.from('codm_events').update(basePayload).eq('id', editingEventId).select('id').single(); error = retry.error; }
    } else {
      const insertPayload = { ...payloadWithPlan, created_by: auth.user?.id || null };
      const insert = await supabase.from('codm_events').insert(insertPayload).select('id').single();
      error = insert.error; eventId = insert.data?.id || null;
      if (error && /event_plan|column/i.test(error.message)) { const retry = await supabase.from('codm_events').insert({ ...basePayload, created_by: auth.user?.id || null }).select('id').single(); error = retry.error; eventId = retry.data?.id || null; }
    }
    if (error) return setMessage(error.message);
    if (eventId) {
      await supabase.from('codm_event_players').delete().eq('event_id', eventId);
      const rows = [...convocati.map((p) => ({ event_id: eventId, clan_id: auth.clanId, player_id: p.id, nickname: p.nickname, status: 'titolare' })), ...reserves.map((p) => ({ event_id: eventId, clan_id: auth.clanId, player_id: p.id, nickname: p.nickname, status: 'riserva' }))];
      if (rows.length) await supabase.from('codm_event_players').insert(rows);
    }
    setMessage(editingEventId ? 'Evento aggiornato. Puoi riaprirlo dal calendario o duplicarlo.' : 'Evento creato con partite ordinate, roster da app, BAN e dettaglio Telegram.');
    setEditingEventId(null);
    await loadEvents();
  }
  async function deleteEvent(id: string) { if (!canWrite) return setMessage('Solo staff/coach/owner possono cancellare eventi.'); if (!confirm('Cancellare evento e convocazioni?')) return; const { error } = await supabase.from('codm_events').delete().eq('id', id); if (error) return setMessage(error.message); setMessage('Evento cancellato.'); await loadEvents(); }

  const eventsByDay = useMemo(() => { const map = new Map<string, CodmEvent[]>(); for (const event of events) { const key = new Date(event.starts_at).toISOString().slice(0, 10); map.set(key, [...(map.get(key) || []), event]); } return map; }, [events]);
  const calendarDays = useMemo(() => { const [year, month] = calendarMonth.split('-').map(Number); const first = new Date(year, month - 1, 1); const start = new Date(first); start.setDate(first.getDate() - ((first.getDay() + 6) % 7)); return Array.from({ length: 42 }, (_, index) => { const day = new Date(start); day.setDate(start.getDate() + index); const key = day.toISOString().slice(0, 10); return { date: day, key, currentMonth: day.getMonth() === month - 1, events: eventsByDay.get(key) || [] }; }); }, [calendarMonth, eventsByDay]);
  ...
  function playersFor(event: CodmEvent, status?: string) { const rows = eventPlayers.filter((r) => r.event_id === event.id && (!status || r.status === status)); if (rows.length) return rows.map((r) => r.nickname); return (event.convocations || []).filter((r: any) => !status || r.role === status).map((r) => r.nickname); }
  ...
  return (
    <main className="container wide ak-page-compact events-v64 events-v65">
      ...
      <section className="card top-gap ak-events-first compact-events-first">
        <div className="section-title"><div><h2>Eventi da fare</h2><p className="muted">Riepilogo compatto. Apri modifica, duplica o importa risultato partita.</p></div><button className="btn secondary small" onClick={() => void loadEvents()}>{eventsLoading ? 'Carico...' : 'Aggiorna'}</button></div>
        <div className="event-presentation-list top-gap">{futureEvents.slice(0, 5).map((event) => <EventPresentation key={event.id} event={event} plan={readPlan(event)} starters={playersFor(event, 'titolare')} reserves={playersFor(event, 'riserva')} canWrite={canWrite} onDelete={deleteEvent} onEdit={() => loadEventIntoEditor(event)} onDuplicate={() => loadEventIntoEditor(event, true)} />)}{!futureEvents.length && <p className="empty-state">Nessun evento futuro in programma.</p>}</div>
      </section>
      ...
      <section className="card top-gap">
        ...
        <div className="ak-calendar-grid">{calendarDays.map((day) => <div key={day.key} className={`ak-calendar-day ${day.currentMonth ? '' : 'muted-month'} ${day.events.length ? 'has-event' : ''}`}><strong>{day.date.getDate()}</strong>{day.events.slice(0, 2).map((event) => <button key={event.id} type="button" className="calendar-event-open" onClick={() => loadEventIntoEditor(event)}>{event.title}</button>)}{day.events.length > 2 && <em>+{day.events.length - 2}</em>}</div>)}</div>
      </section>

      <section className="top-gap">
        {canWrite ? <div className="card event-create-v64">
          <div className="section-title"><div><h2>{editingEventId ? 'Modifica evento / Editor partite' : 'Nuovo evento / Editor partite'}</h2><p className="muted">I campi risultato sono aggiornati dall’import partita. La bozza resta salvata anche se cambi tab Chrome.</p></div><div className="editor-actions-row"><button className="btn small" type="button" onClick={addRound}>+ Aggiungi partita</button><button className="btn small secondary" type="button" onClick={() => resetEditor(false)}>Nuovo</button><button className="btn small secondary" type="button" onClick={() => resetEditor(true)}>Reset bozza</button></div></div>
          <div className="match-count-toolbar top-gap"><span>Partite configurate: <b>{plan.rounds.length}</b></span><button className="btn small secondary" type="button" onClick={() => removeRound(plan.rounds.length - 1)} disabled={plan.rounds.length <= 1}>- Togli ultima partita</button><small>Default 1 partita. Ogni partita è lavorabile e separata.</small></div>
          ...
          <div className="form top-gap">
            ...
            <div className="field"><label>Messaggio Telegram</label><textarea className="input" rows={7} value={telegramTemplate} onChange={(e) => setTelegramTemplate(e.target.value)} /><small className="muted">Usa <b>{'{match_details}'}</b> per inviare Partita 1, Partita 2, ecc. con dettagli, orari e convocati.</small></div>
            ...
            <button className="btn" onClick={() => void createEvent()}>{editingEventId ? '💾 Aggiorna evento' : 'Crea evento completo'}</button>
          </div>
      ...
      <section className="card top-gap">
        <div className="section-title"><div><h2>Archivio eventi</h2><p className="muted">Caricati: {events.length} • futuri: {futureEvents.length} • passati: {pastEvents.length}</p></div><select className="select compact-select" value={eventFilter} onChange={(e) => setEventFilter(e.target.value as 'future' | 'all' | 'past')}><option value="future">Futuri</option><option value="all">Tutti</option><option value="past">Passati</option></select></div>
        <div className="ak-event-list top-gap">{visibleEvents.map((event) => { const googleUrl = event.google_calendar_url || buildGoogleCalendarUrl({ title: event.title, description: event.description || '', location: event.location || '', startsAt: event.starts_at, endsAt: event.ends_at }); return <article key={event.id} className="ak-event-card"><div><div className="eyebrow">{event.event_type || 'evento'}</div><h3>{event.title}</h3><p className="muted">{new Date(event.starts_at).toLocaleString('it-IT')} {event.location ? `• ${event.location}` : ''}</p></div><div className="ak-event-actions"><button className="btn secondary" onClick={() => loadEventIntoEditor(event)}>Modifica</button><button className="btn secondary" onClick={() => loadEventIntoEditor(event, true)}>Duplica</button><a href={googleUrl} target="_blank" rel="noreferrer" className="btn secondary">Google Calendar</a>{canWrite && <button className="btn danger secondary" onClick={() => void deleteEvent(event.id)}>Cancella</button>}</div></article>; })}{!visibleEvents.length && <p className="empty-state">Nessun evento da mostrare.</p>}</div>
      </section>
    </main>
  );
}

function PlayerPicker({ title, players, selected, toggle }: { title: string; players: PlayerRow[]; selected: string[]; toggle: (id: string) => void }) { return <div className="field"><label>{title}</label><div className="ak-player-pick-list">{players.map((player) => <label key={player.id} className="ak-player-pick"><input type="checkbox" checked={selected.includes(player.id)} onChange={() => toggle(player.id)} /><span>{player.nickname}</span><small>{player.clan_name || 'AK47DX'}</small></label>)}{!players.length && <p className="muted">Nessun player nel roster.</p>}</div></div>; }
function RoundRosterPicker({ title, players, selected, opposite, onToggle }: { title: string; players: PlayerRow[]; selected: string[]; opposite: string[]; onToggle: (nickname: string) => void }) { return <div className="round-roster-box"><label>{title}</label><div className="round-roster-picks">{players.map((player) => { const isSelected = selected.includes(player.nickname); const isOpposite = opposite.includes(player.nickname); return <button key={player.id} type="button" className={`round-roster-chip ${isSelected ? 'active' : ''} ${isOpposite ? 'opposite' : ''}`} onClick={() => onToggle(player.nickname)} title={isOpposite ? 'Selezionando qui verrà rimosso dall’altro gruppo' : player.nickname}>{isSelected ? '✓ ' : ''}{player.nickname}</button>; })}{!players.length && <span className="muted">Nessun player registrato.</span>}</div></div>; }
function BanPicker({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  const [custom, setCustom] = useState('');
  const selected = listFromText(value);
  function toggleBan(item: string) { onChange(selected.includes(item) ? textFromList(selected.filter((x) => x !== item)) : textFromList([...selected, item])); }
  function addCustom() { const clean = custom.trim(); if (!clean) return; onChange(textFromList([...selected, clean])); setCustom(''); }
  return <div className="ban-picker"><div className="ban-chip-list">{banOptions.map((item) => <button key={item} type="button" className={`ban-chip ${selected.includes(item) ? 'active' : ''}`} onClick={() => toggleBan(item)}>{selected.includes(item) ? '✓ ' : '🚫 '}{item}</button>)}</div><div className="ban-custom-row"><input className="input" value={custom} onChange={(e) => setCustom(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addCustom(); } }} placeholder="Aggiungi BAN manuale" /><button className="btn small secondary" type="button" onClick={addCustom}>Aggiungi</button></div><small className="muted">BAN selezionati: {selected.length ? selected.join(', ') : 'nessuno'}</small></div>;
}
...
function EventPresentation({ event, plan, starters, reserves, canWrite, onDelete, onEdit, onDuplicate }: { event: CodmEvent; plan: MatchPlan; starters: string[]; reserves: string[]; canWrite: boolean; onDelete: (id: string) => Promise<void>; onEdit: () => void; onDuplicate: () => void }) {
  const normalizedPlan = normalizePlan(plan); const rounds = normalizedPlan.rounds.slice(0, Number(normalizedPlan.totalMatches || normalizedPlan.rounds.length || 1));
  return <article className="event-presentation-card event-presentation-card-v65">
...
function TeamLogo({ name, logo }: { name: string; logo?: string }) { return <div className="team-logo-card">{logo ? <img src={logo} alt={name} /> : <div className="team-logo-placeholder">{name.slice(0, 2).toUpperCase()}</div>}<strong>{name}</strong></div>; }
